...
{
    public partial class FrmJohnsonThemCanh : DevExpress.XtraEditors.XtraForm
    {
        public FrmJohnsonThemCanh()
        {
            InitializeComponent();
        }
        private void LoadInitControl()
        {
            List<Dinh> xp = Data.Data.graph_Johnson.dsDinh.ToList();
            // cbx Dinh Xuat Phat
            cbxDinhXP.Properties.DataSource = xp;
            cbxDinhXP.Properties.DisplayMember = "Ten";
            cbxDinhXP.Properties.ValueMember = "ID";

            List<Dinh> kt = Data.Data.graph_Johnson.dsDinh.ToList();
            // cbx Dinh Ket Thuc
            ...
            cbxLoaiCanh.Text = "Cạnh một chiều";
        }

        private void FrmJohnsonThemCanh_Load(object sender, EventArgs e)
        {
            LoadInitControl();
        }
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            if (Check())
            {
                ...
                int cnt = Data.Data.graph_Johnson.dsCanh.Count;
                if (cnt == 0) canh.ID = 1;
                else canh.ID = Data.Data.graph_Johnson.dsCanh[cnt - 1].ID + 1;

                canh.IDXP = (int)cbxDinhXP.EditValue;
                canh.IDKT = (int)cbxDinhKT.EditValue;
                canh.x1 = Data.Data.graph_Johnson.dsDinh.Where(p => p.ID == canh.IDXP).FirstOrDefault().x;
                canh.y1 = Data.Data.graph_Johnson.dsDinh.Where(p => p.ID == canh.IDXP).FirstOrDefault().y;
                canh.x2 = Data.Data.graph_Johnson.dsDinh.Where(p => p.ID == canh.IDKT).FirstOrDefault().x;
                canh.y2 = Data.Data.graph_Johnson.dsDinh.Where(p => p.ID == canh.IDKT).FirstOrDefault().y;
                canh.TrongSo = Int32.Parse(txtTrongSo.Text);
                canh.mau = Color.Black;
                canh.LoaiCanh = LoaiCanh;

                Data.Data.graph_Johnson.dsCanh.Add(canh);
                if (KTChuTrinhAm.isNegCycleBellmanFord(Data.Data.graph_Johnson))
                {
                    MessageBox.Show("Đồ thị có chu trình ầm", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Data.Data.graph_Johnson.dsCanh.RemoveAll(p => p.ID == canh.ID);
                }
                else
